...
from oauth.filters import OrganizationFilter
from .api import Api
from .models import WorkerInvoice
from .forms import (
    WorkerInvoiceCreateForm, WorkerInvoiceReadOnlyForm, ChooseMedClientWorkerForm
)
...
class WorkerInvoiceAdmin(ModelAdmin):
    ...
    list_filter = [WorkerOrganizationFilter, "worker__type"]
    list_filter_submit = True
    list_filter_sheet = False

    def get_form(self, request, obj, *args, **kwargs):
        if obj:
            kwargs['form'] = WorkerInvoiceReadOnlyForm
        else:
            if request.GET.get('medClientId'):
                kwargs['form'] = WorkerInvoiceCreateForm
            else:
                kwargs['form'] = ChooseMedClientWorkerForm
